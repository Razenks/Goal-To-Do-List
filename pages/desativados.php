...
$sql = "SELECT id_card, titulo, descricao, data_inicio, data_prazo, importancia, estado
            FROM cards
            WHERE estado = 3 AND cpf = :cpf";
...
<head>
    ...
    <link rel="stylesheet" href="../css/desativar.css">
    ...
    <main>
        ...
        if (!empty($cards)) {
            foreach ($cards as $card) {
                ...
                echo '<div>';
                echo '<button><a href="card_page.php?id_card=' . $card['id_card'] . '">Ver</a></button>';
                echo '</div>';
                echo '</div>';
                echo '</div>';
            }
